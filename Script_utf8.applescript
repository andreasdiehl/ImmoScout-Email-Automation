...
on run
	...
	try
		...
		if my trim(latestVersion) > scriptVersion then
			display dialog "🚀 Neue Version verfügbar: " & latestVersion & return & "Installiert: " & scriptVersion & return & return & "Möchtest du updaten?" buttons {"Später", "Zum Download"} default button "Zum Download"
...
on ladeConfig()
	try
		...
		on error errMsg
			...
			display dialog "⚙️ config.txt wurde erstellt, weil keine vorhanden war." & return & return & "Bitte config.txt bearbeiten und Script neu starten." & return & return & "Details: " & errMsg buttons {"OK"}
			return false
		end try
	...
	on error errMsg
		display dialog "❌ Fehler beim Laden der config.txt." & return & return & "Details: " & errMsg buttons {"OK"}
		return false
	end try
end ladeConfig
...
# 10. NUR DIESE SCOUT-IDs BEARBEITEN (leer = alle)
scoutIds=
"
	...
	try
		...
		display dialog "⚙️ Config-Datei erstellt!" & return & return & "Bitte bearbeite:" & return & "config.txt" & return & return & "im gleichen Ordner wie das Script." & return & return & "Danach Script neu starten." buttons {"OK"}
		
	on error errMsg
		try
			close access file configPfad
		end try
		display dialog "❌ Konnte config.txt nicht erstellen." & return & return & "Details: " & errMsg buttons {"OK"}
	end try
...
on logPfad()
	-- Log liegt im gleichen Ordner wie das Script
	set scriptPfad to path to me
	tell application "Finder"
		set scriptOrdner to container of scriptPfad as alias
	end tell
	return ((scriptOrdner as text) & logDateiName)
end logPfad
